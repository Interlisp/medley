...
[Run]
; Filename: "{app}\cygwin\setup-x86_64.exe"; Parameters: "--quiet-mode --no-admin --wait --no-shortcuts --no-write-registry --verbose --root ""{app}"" --site https://mirrors.kernel.org/sourceware/cygwin --only-site --local-package-dir ""{app}\cygwin"" --packages nano,xdg-utils,ghostscript,ghostscript-fonts-other,binutils,clang,cmake,gcc-core,git,libX11-devel,make,pkgconf,xinit,xorg-server"; StatusMsg: "Installing Cygwin ..."
Filename: "{app}\cygwin\setup-x86_64.exe"; Parameters: "--quiet-mode --no-admin --wait --no-shortcuts --no-write-registry --verbose --root ""{app}"" --site https://mirrors.kernel.org/sourceware/cygwin --only-site --local-package-dir ""{app}\cygwin"" --packages nano,xdg-utils,ghostscript,ghostscript-fonts-other,libX11_6,xinit,xorg-server,xdpyinfo,xclip"; StatusMsg: "Installing Cygwin ..."
