...
main() {
	...
	(PROG
	  ((WORKDIR (IL:CONCAT \"{DSK}\" (IL:UNIX-GETENV \"LOADUP_WORKDIR\") \"/\")))
          (IL:PUTASSOC (QUOTE IL:FULLER)
                       (LIST (IL:UNIX-GETENV \"LOADUP_COMMIT_ID\"))
                       (ASSOC (QUOTE IL:MEDLEY) IL:SYSOUTCOMMITS))
	  (IL:MEDLEY-INIT-VARS)
	  (IL:FILESLOAD MEDLEY-UTILS)
	  (SETQ IL:DIRECTORIES (CONS (IL:UNIX-GETENV \"LOADUP_SOURCEDIR\") IL:DIRECTORIES))
	  (IL:MAKE-FULLER-DB
	    (IL:CONCAT WORKDIR \"fuller.dribble\")
	    (IL:CONCAT WORKDIR \"fuller.database\")
	    (IL:CONCAT WORKDIR \"fuller.sysout\")
	  )
	)
        (IL:LOGOUT T)
