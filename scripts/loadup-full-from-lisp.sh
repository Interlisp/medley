...
main() {
	...
	(PROGN
	  ...
	  (IL:LOADUP-FULL (IL:CONCAT (QUOTE {DSK}) (IL:UNIX-GETENV (QUOTE LOADUP_WORKDIR))
                          (IL:L-CASE (QUOTE /full.dribble)))
	  (IL:PUTASSOC (QUOTE IL:FULL)
                       (LIST (IL:UNIX-GETENV (QUOTE LOADUP_COMMIT_ID)))
                       (CADR (ASSOC (QUOTE IL:MEDLEY) IL:SYSOUTCOMMITS)))
	  (IL:ENDLOADUP)
	  (IL:MAKESYS (IL:CONCAT (QUOTE {DSK}) (IL:UNIX-GETENV (QUOTE LOADUP_WORKDIR)) (IL:L-CASE (QUOTE /full.sysout)))
                      :FULL))
	)
       (IL:LOGOUT T)
