...
main() {
	...
	(PROGN
	  (LOAD (CONCAT %"{DSK}%" (UNIX-GETENV %"MEDLEYDIR%") %"/sources/MEDLEYDIR.LCOM%"))
	  (MEDLEY-INIT-VARS)
	  (LOAD (CONCAT %"{DSK}%" (UNIX-GETENV %"LOADUP_SOURCEDIR%") %"/LOADUP-LISP.LCOM%"))
	  (LOADUP-LISP (CONCAT %"{DSK}%" (UNIX-GETENV %"LOADUP_WORKDIR%") %"/lisp.dribble%"))
	  (SETQ SYSOUTCOMMITS (LIST (LIST 'MEDLEY (LIST (LIST 'INIT (UNIX-GETENV 'LOADUP_COMMIT_ID))))))
	  (PUTASSOC (QUOTE LISP)
                    (LIST (UNIX-GETENV %"LOADUP_COMMIT_ID%"))
                    (CADR (ASSOC (QUOTE MEDLEY) SYSOUTCOMMITS)))
	  (IL:ENDLOADUP)
	  (IL:MAKESYS (IL:CONCAT %"{DSK}%" (IL:UNIX-GETENV %"LOADUP_WORKDIR%") %"/lisp.sysout%") :LISP)
  	)
        (LOGOUT T)
